...
//checking if request cotains multipart data
    ...
    if (isMultipart) {
        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("filephoto"))//write the name of image here it is img_product
                {
                    ...
                    photo = "AgencyPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                ...
                if (field_name.equals("fileproof"))//write the name of image here it is img_product
                {
                    ...
                    license = "Agencylicense_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                    //creating a file object
                    savedFile = new File(file_path + license);//fn contains name of file
                    try {
                        //writing the file object
                        f_item.write(savedFile);  //writing to server means the folder             
        ...
        if (value[6].equals(value[7])) {
            String InsQry = "INSERT INTO tbl_agency( agency_name,agency_address,agency_contact,agency_photo,agency_license,agency_email,place_id,agency_password)"
                    + "values('" + value[0] + "','" + value[1] + "','" + value[2]  + "','" + photo + "','" + license + "','" + value[3] + "','" + value[5] + "','" + value[6] + "')";
